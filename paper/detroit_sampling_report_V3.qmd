---
title: "Detroit Transportation Multi-stage Survey Design"
...
author-note:
  disclosures:
    conflict-of-interest: The authors declare no conflicts of interest. This technical report submiting on April 20, 2026.
format:
  apaquarto-pdf:
    documentmode: man
    keep-tex: false
...
The joint BG probability is $\pi_{ij} = \pi_i \cdot \pi_{j|i}$. Across the 40 sample block groups, $\pi_{ij}$ ranges from `r round(pi_ij_range[1], 4)` to `r round(pi_ij_range[2], 4)`, all below 1. Within-BG person rates $\pi_{k|ij}(d) = \bar{q}f_d/S_{ij}$ vary across block groups: 0.027 to 0.120 for 18--34, 0.030 to 0.133 for 35--49, 0.026 to 0.115 for 50--64, and 0.027 to 0.119 for 65+, all below 1, which confirms feasibility. The expected workload per BG equals `r round(q_bar, 2)` in every sample block group, which confirms the equal-workload property.
