...
cat("pi_i (PSU selection probability) range:", 
...
ssu_frame <- ssu_frame |>
  mutate(MOS_bg = rowSums(across(all_of(domain_cols)) * f_d))
```

## VDK 10.1 verification table {#sec-vdk-verify}

-   Following the spreadsheet layout in VDK Example 10.1 (Table 10.1, p. 284), we verify four properties at the PSU level. These are the same checks I performed in Assignment 8 (VDK Exercise 10.2) using the two-domain example, extended here to four domains and applied to our Detroit frame.
...
cat("What is the nworkload per BG? (should =", round(q_bar, 1), "):\n")
